...
const SOMA_BUILD_ID = 'rollback-20260401-02';
console.info('[SOMA] build', SOMA_BUILD_ID);
...
const el = {
  ...
  btnAddTransaction: byId('btnAddTransaction'),
...
function bindEvents() {
  ...
  if (el.selectedMonth) {
    el.selectedMonth.addEventListener('change', async (e) => {
      ...
      if (el.txDate) el.txDate.value = `${state.selectedMonth}-01`;
...
async function addTransaction() {
  ...
  if (!String(occurred_on).startsWith(state.selectedMonth)) {
    showAppMessage('A data do lançamento precisa estar dentro do mês de referência selecionado.', 'error');
    return;
  }

  setButtonLoading(el.btnAddTransaction, true, 'Salvando');
  ...
  try {
    ...
    console.error('[SOMA addTransaction]', error);
    showAppMessage('Falha inesperada ao lançar o gasto.', 'error');
  } finally {
    setButtonLoading(el.btnAddTransaction, false);
  }
}
...
function renderApp() {
  ...
  renderOnboardingStep();
  if (el.txDate && (!el.txDate.value || !String(el.txDate.value).startsWith(state.selectedMonth))) {
    el.txDate.value = `${state.selectedMonth}-01`;
  }
  renderCategoryOptions();
  renderDashboard();
